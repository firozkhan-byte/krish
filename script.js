...
(function () {
  ...
  /* ------------------------------------------------------------------
     14. Certificates slider — arrow buttons + auto-scroll
     Pauses on hover/touch/focus and after a manual arrow click, then
     resumes. Skips auto-scroll entirely for prefers-reduced-motion.
     ------------------------------------------------------------------ */
  const certScroll = doc.getElementById('certDocScroll');
  const certPrev = doc.getElementById('certPrev');
  const certNext = doc.getElementById('certNext');

  if (certScroll && certPrev && certNext) {
    const getStep = () => {
      const card = certScroll.querySelector('.cert-doc-card');
      const grid = certScroll.querySelector('.cert-doc-grid');
      const gap = grid ? parseFloat(getComputedStyle(grid).columnGap) || 0 : 0;
      return card ? card.getBoundingClientRect().width + gap : 220;
    };

    const scrollByStep = (dir) => certScroll.scrollBy({ left: dir * getStep(), behavior: 'smooth' });

    certPrev.addEventListener('click', () => { scrollByStep(-1); pauseThenResume(); });
    certNext.addEventListener('click', () => { scrollByStep(1); pauseThenResume(); });
    ...
    function startAuto() {
      ...
      autoTimer = setInterval(() => {
        const atEnd = certScroll.scrollLeft + certScroll.clientWidth >= certScroll.scrollWidth - 4;
        if (atEnd) certScroll.scrollTo({ left: 0, behavior: 'smooth' });
        else scrollByStep(1);
      }, 3000);
    }
    function stopAuto() {
    ...
    ['mouseenter', 'touchstart', 'focusin'].forEach((evt) =>
      certScroll.addEventListener(evt, stopAuto, { passive: true })
    );
    ['mouseleave', 'touchend'].forEach((evt) =>
      certScroll.addEventListener(evt, startAuto, { passive: true })
    );
